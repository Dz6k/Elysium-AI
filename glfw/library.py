...
if getattr(sys, 'frozen', False):
    ...
    # O caminho normal para desenvolvimento
...
def resource_path2(relative_path):
    base_path = getattr(
        sys,
        '_MEIPASS',  # Usado quando empacotado pelo PyInstaller
        os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))  # Caminho da raiz do projeto
    )
...
if os.environ.get('PYGLFW_LIBRARY', ''):
    ...
    try:
        glfw = ctypes.CDLL(resource_path2('glfw\glfw3.dll'))
    except OSError:
        pass
    ...
    if glfw is None:
        try:
            ...
            msvcr = ctypes.CDLL(resource_path2('glfw\msvcr120.dll'))
